...
def crop_to_vertical(input_video_path, output_video_path):
    ...
    for _ in range(total_frames):
        ...
        # Initialize default values in case no faces are detected
        (x, y, w, h) = (0, 0, 0, 0)  
        ...
        # Retrieve the corresponding frame data if available
        if count < len(Frames):
            try:
                (X, Y, W, H) = Frames[count] 
                # Additional logic to adjust cropping based on face positions
                centerX = X + (W // 2)

                # Adjust x_start and x_end based on the detected face
                if count == 0 or (x_start - (centerX - half_width)) < 1:
                ...
                x_start, x_end = (0, original_width)

        cropped_frame = frame[:, x_start:x_end]
        if cropped_frame.shape[1] <= 0:  # Fallback if cropping fails
            x_start = (original_width - vertical_width) // 2
            x_end = x_start + vertical_width
            cropped_frame = frame[:, x_start:x_end]
...
def combine_videos(video_with_audio, video_without_audio, output_filename):
    try:
        ...
        # Extract audio from the video with audio
        audio = clip_with_audio.audio

        # Check if the audio exists
        if audio is None:
            print("No audio found in video_with_audio.")
            return

        # Combine the audio with the video without audio
        combined_clip = clip_without_audio.set_audio(audio)
