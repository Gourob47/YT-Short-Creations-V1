...
#             #     face_width = x1 - x
#             #     face_height = y1 - y
#             if confidence > 0.3:  # Inside your face detection loop
#                 box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
#                 (x, y, x1, y1) = box.astype("int")
    ...
    while cap.isOpened():
        ...
        for i in range(detections.shape[2]):
            ...
            if confidence > 0.3:  # Confidence threshold
                ...
                face_width = x1 - x
                face_height = y1 - y  # Calculate only if a face is detected
        ...
        # Ensure lip_distance is calculated only when a face is detected
        if Add:
            for (face_coords, lip_distance) in Add:
                (x, y, x1, y1) = face_coords
                if lip_distance >= MaxDif and is_speaking_audio:
                    cv2.putText(frame, "Active Speaker", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Only append valid frames to avoid UnboundLocalError
        if Add:
            (x, y, x1, y1) = Add[0][0]  # Use the first detected face
            Frames.append([x, y, x1, y1])
